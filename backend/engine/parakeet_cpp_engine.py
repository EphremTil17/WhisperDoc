"""Parakeet.cpp sidecar adapter.

The native ggml/CUDA runtime lives in a separate container.  This adapter keeps
WhisperDoc's Python process free of NeMo, PyTorch, and native model bindings
while preserving the synchronous :class:`BaseEngine` contract used by the
...
class ParakeetCppEngine(BaseEngine):
    ...
    def __init__(
        ...
        *,
        ...
        log.info(
            "ParakeetCppEngine: connecting to isolated sidecar at {}",
            self._base_url,
        )
        self.warmup()
    ...
    def _validate_base_url(value: str) -> str:
        ...
        if parsed.scheme not in {"http", "https"} or not parsed.netloc:
            raise ValueError("PARAKEET_CPP_BASE_URL must be an absolute HTTP(S) URL.")
        if parsed.username or parsed.password or parsed.query or parsed.fragment:
            raise ValueError(
                "PARAKEET_CPP_BASE_URL cannot contain credentials, a query, or a fragment."
            )
        return normalized
    ...
    def warmup(self) -> None:
        with self._state_lock:
            ...
            self._ready = True
        log.success("ParakeetCppEngine: sidecar healthy and CUDA path warmed")
    ...
    def unload(self) -> None:
        ...
        with self._state_lock:
            self._ready = False
        log.info("ParakeetCppEngine: detached from externally managed sidecar")
