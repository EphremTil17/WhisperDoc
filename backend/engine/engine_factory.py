...
each runtime loads only the implementation selected for this process.
"""
...
def create_engine() -> BaseEngine:
    ...
    Reads ASR_ENGINE from the environment:
      ...
      'parakeet' -> ParakeetEngine backed by an isolated native sidecar
    ...
    if engine_name == "parakeet":
        ...
        return ParakeetEngine(
            base_url=os.getenv("PARAKEET_BASE_URL", "http://parakeet:8080"),
            connect_timeout_seconds=float(
                os.getenv("PARAKEET_CONNECT_TIMEOUT_SECONDS", "2")
            ),
            request_timeout_seconds=float(
                os.getenv("PARAKEET_REQUEST_TIMEOUT_SECONDS", "30")
            ),
            startup_timeout_seconds=float(
                os.getenv("PARAKEET_STARTUP_TIMEOUT_SECONDS", "120")
            ),
            warmup_seconds=float(os.getenv("PARAKEET_WARMUP_SECONDS", "10")),
            max_audio_seconds=float(os.getenv("PARAKEET_MAX_AUDIO_SECONDS", "30")),
        )

    raise ValueError(
        f"Unknown ASR_ENGINE: '{engine_name}'. "
        "Valid options are: 'whisper', 'parakeet'."
    )
