...
def generate_test_jwt(
    ...
    audience: str | list[str] = "whisperdoc_client",
    ...
    **extra_claims,
    ...
    token = jwt.encode(
        payload, private_key, algorithm="RS256", headers={"kid": "test_key_id_001"}
    )
...
def generate_mock_jwks():
    ...
    if not isinstance(public_key, rsa.RSAPublicKey):
        raise TypeError("Test JWKS fixture expected an RSA public key.")

    # Extract modulus and exponent
    numbers = cast(rsa.RSAPublicNumbers, public_key.public_numbers())
