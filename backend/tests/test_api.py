...
def test_http_transcribe_dummy():
    ...
    if not API_KEY:
        pytest.skip(
            "WHISPER_DOC_API_KEY not set; required to reach validation logic on the live server"
        )
...
def test_http_transcription_real():
    ...
    if not os.path.exists(audio_file):
        pytest.skip(
            f"Test file {audio_file} not found. Skipping real transcription test."
        )

    with open(audio_file, "rb") as f:
        files = {"file": (os.path.basename(audio_file), f, "audio/wav")}
        response = requests.post(
            f"{API_URL}/transcribe", files=files, headers=get_headers(), timeout=30
        )
    ...
    if check_server():
        ...
        if args.file and os.path.exists(args.file):
            ...
            with open(args.file, "rb") as f:
                ...
                response = requests.post(
                    f"{API_URL}/transcribe", files=files, timeout=30
                )
                if response.status_code == 200:
                    print(
                        f"✅ Transcription Success: {response.json()['text'][:100]}..."
                    )
                else:
