...
def test_jwt_valid_authentication():
    ...
    with patch.dict(
        ...
        with (
            patch("auth.oidc.requests.get") as mock_get,
            patch(
                "auth.oidc.OIDC_ISSUER_URL",
                "https://auth.test.local/application/o/test/",
            ),
...
def test_jwt_wrong_issuer():
    ...
    with patch.dict(
        ...
        with (
            ...
            token = generate_test_jwt(
                issuer="https://evil.attacker.com/", audience="whisperdoc_client"
            )
