...
# --- Configuration ---
IDLE_TIMEOUT_SECONDS = int(os.getenv("IDLE_TIMEOUT_SECONDS", "375"))
HANDSHAKE_TIMEOUT_SECONDS = int(os.getenv("HANDSHAKE_TIMEOUT_SECONDS", "15"))
NO_AUDIO_GRACE_SECONDS = int(os.getenv("NO_AUDIO_GRACE_SECONDS", "360"))
...
class SecurityGovernance:
    ...
    def is_ip_banned(self, ip: str) -> Tuple[bool, int]:
        ...
        if ban_until:
            ...
            if now < ban_until:
                ...
                if ip in self.attempt_tracker:
                    del self.attempt_tracker[ip]
    ...
    def record_connection_attempt(self, ip: str):
        ...
        if len(attempts) > FLOOD_ATTEMPT_THRESHOLD:
            log.error(
                f"IP {ip} flagged for Connection Spamming. Banning for {FLOOD_BAN_SECONDS}s."
            )
    ...
    def record_protocol_violation(self, ip: str):
        ...
        if len(failures) >= BAN_FAILURE_THRESHOLD:
            log.error(
                f"IP {ip} exceeded failure threshold ({BAN_FAILURE_THRESHOLD} in {BAN_FAILURE_WINDOW_SECONDS}s). Banning for {BAN_DURATION_SECONDS}s."
            )
    ...
    def clear_old_trackers(self):
        ...
        self.failure_tracker = {
            ip: ts for ip, ts in self.failure_tracker.items() if now - ts[-1] < 3600
        }
        self.attempt_tracker = {
            ip: ts for ip, ts in self.attempt_tracker.items() if now - ts[-1] < 3600
        }
